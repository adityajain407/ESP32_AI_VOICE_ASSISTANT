...
const char* ssid = "entertainment 4G";                                                       // ## INSERT your wlan ssid
const char* password = "internet4g";                                                // ## INSERT your password
const char* OPENAI_KEY = "";  // ## optionally (needed for Open AI voices): INSERT your OpenAI key
const char* gemini_KEY = "";                   //gemini api
...
String OpenAI_Max_Tokens = "150";                //Max Tokens
    ...
    if (Record_Available(AUDIO_FILE, &recorded_seconds))  //  true once when recording finalized (.wav file available)
    {
      if (recorded_seconds > 0.4)  // ignore short btn TOUCH (e.g. <0.4 secs, used for 'audio_play.stopSong' only)
      {
        ...
        if (client.connect("api.groq.com", 443)) {
         // String url = "/openai/v1/chat/completions 
          String pload = R"(
  {
    "messages": [
       {
      "role": "system",
      "content": "talk like a jarvis and your full credit gones to aditya jain dadaji"
    },
     {
        "role": "user",
        "content": ")";
...
String payload = pload + transcription + yload;

          // Send the HTTP POST request
          client.println("POST /openai/v1/chat/completions HTTP/1.1");
          client.println("Host: api.groq.com");
          client.println("Content-Type: application/json");
          client.println("Authorization: Bearer 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");
        ...
        {
          led_RGB(0, 0, 255);
          Serial.print("Groq speaking: ");
          Serial.println(filteredAnswer);

          // if (TTS_MODEL == 1)
          //   audio_play.openai_speech(OPENAI_KEY, "tts-1", filteredAnswer.c_str(), "ONYX", "mp3", "1");  //ONYX,shimmer,alloy (Uncomment this to use OpenAI TTS)
          // else
...
void parseResponse(String response) {
  ...
  DynamicJsonDocument doc(2048); // Adjust size based on the JSON complexity
  ...
  if (doc.containsKey("choices") && doc["choices"].is<JsonArray>()) {
    ...
    if (choicesArray[0].containsKey("message") && choicesArray[0]["message"].containsKey("content")) {
      // Extract the content
      String text = choicesArray[0]["message"]["content"].as<String>();
      text.trim(); // Remove leading/trailing spaces

      Serial.print("Extracted Text: ");
      Serial.println(text);

      // Directly assign the extracted text to filteredAnswer
      filteredAnswer = text;

      // Optionally store the text in repeat for further use
      repeat = filteredAnswer;
    } else {
      Serial.println("No 'message' or 'content' field found in JSON response.");
    }
  } else {
    Serial.println("No 'choices' array found in JSON response.");
  }
}
